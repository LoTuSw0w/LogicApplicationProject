...
namespace LogiX
{
    class DNF
    {
        ...
        public DNF(List<int> results, List<char[]> values, string label)
        {
            List<string> allOne = GenerateListForDNF(results, values);
            if(allOne.Count > 0)
            {
                dnfString = processAllLines(allOne, label);
            }
            else
            {
                dnfString = "Cannot generate a DNF with a contradiction!";
            }
        }


        public List<string> GenerateListForDNF(List<int> allResults, List<char[]> allValues)
        {
            ...
            for (int i = 0; i < allResults.Count; i++)
            {
                if (allResults[i] == 1)
                {
                    toReturn.Add(new String(allValues[i]));
                }
            }
            return toReturn;
        }
        ...
        //
        //These two functions run the same way, maybe I will find a way to streamline the process
